...
{   Param (
    ...
    #Done, now output:
    if ( $outputpath -eq "screen") {write-output $Result }
        else { $result | Out-File $OutputPath }
}
...
try { $ipaddr = [System.Net.DNS]::GetHostEntry($computer).AddressList }
    catch { $wshell.Popup("Error: Address not found.",0,"Error",0)
        exit }

foreach ($addr in $ipaddr) 
    { if ($addr -match "^.*\.\.*") { $PrintAddress = $addr} }

if (Test-Connection $PrintAddress -count 1 -quiet) {
    try {Get-Telnet -Port 4114 -RemoteHost "plsdc02.students.tower" -commands "login", "login admin readwrite", "log off $($printaddress)" 
        $wshell.Popup("Reset $($computer)",0,"Success",0)}
    catch { $wshell.Popup("Somethign went wrong",0,"NOPE",0x1)}
    }
